...
public class EventsDao {
	...
	public List<Adiacenza> getArchi(String categoria, int mese){
		String sql = "select e1.offense_type_id as v1, e2.offense_type_id as v2, COUNT(DISTINCT e1.neighborhood_id) as peso "
				...
				+ "		e1.offense_category_id = ? and e1.offense_category_id = e2.offense_category_id and MONTH(e1.reported_date) = ? and MONTH(e1.reported_date) = MONTH(e2.reported_date) and e1.neighborhood_id = e2.neighborhood_id "
		...
		try {
			...
			ResultSet res = st.executeQuery() ;
			while (res.next()) {
				archi.add(new Adiacenza(res.getString("v1"), 
